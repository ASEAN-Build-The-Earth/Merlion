        ...
        axios.get('https://mcapi.us/server/status?ip=asean.my.to')
            .then(response => {
                ...
                const srv = new Discord.MessageEmbed()
                    ...
                    .addFields({ name: "Server Online?", value: `${ServerOnline}` }, { name: "Player count", value: `${response.data.players.now} /${response.data.players.max}` }, { name: "Request", value: `${response.data.status}` }, )
