...
class AddAPICommand extends Command {
	...
	async messageRun(message, args) 
	{
        ...
        const error = (errorEmbed) => get(message).edit({ embeds: [errorEmbed] });
        ...
        registerInput(message.attachments, argsContext[0], nestedName, 
        // generate registered data takes time so we use callback function for everything
        (registeredData) => {

            if(registeredData.error !== null) {
                const errorEmbed = new MessageEmbed().setDescription(registeredData.error).setColor("#ee0004");
                return error(errorEmbed);
            }
...
function saveApi(api, comfirmation, message)
{
    ...
    switch (comfirmation.response) 
    {
        case "yes":
        {
            ...
            (result) => {
                if(typeof result[0].data === "string")    //!result[0].data.link.match(/^https\:\/\/i\.imgur\.com\//i)
                {
                    confirmationEmbed.setTitle("Error saving image! please try again later").setDescription(`on imgur saving process: ${result[0].data}`).setColor("#89eb34")
...
async function registerInput(attachments, inputType, name, callback)
{
    ...
    if(f_name !== null) // fetch the name if name has been used before 
    {
        getName(Api, f_name, (data) => {
            if(data.length > 0)
                f_error = `Sorry, the name \`${f_name}\` has already been used before`;
            return callback(fetchFilter());
        });
    }
    else return callback(fetchFilter());  
}
